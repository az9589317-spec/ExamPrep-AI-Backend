...
const neetSubCategories = subCategories.NEET.map(name => ({
    name,
    description: `Exams and tests for NEET ${name}.`
}));
...
export default async function NeetPage() {
  ...
  const examCounts = await (async () => {
    ...
    for (const subCategory of neetSubCategories) {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col">
      ...
      <main className="flex-1 p-4 md:p-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold font-headline tracking-tight">NEET Exams</h1>
          <p className="mt-2 text-lg text-muted-foreground">Choose a specific NEET exam to view available tests.</p>
        </div>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {neetSubCategories.map((subCategory) => (
